...
export default function LeadCaptureModal({ delay = 0, sessionKey = 'leadModalSeen', showEveryVisit = false }: LeadCaptureModalProps) {
  ...
  useEffect(() => {
    try {
      if (showEveryVisit) {
        // Always show on mount (for landing pages)
        if (delay > 0) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-8">
        <h2 className="text-2xl font-bold text-text-heading mb-2">Get Premium Trading Signals</h2>
        <p className="text-text-body mb-6">Enter your details to get started with expert-curated trading insights.</p>
        ...
        <button onClick={handleClose} className="w-full text-center text-sm text-gray-400 mt-4 hover:text-gray-600">
          Skip for now
        </button>
      </div>
    </div>
